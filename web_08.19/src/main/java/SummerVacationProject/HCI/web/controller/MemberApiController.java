...
public class MemberApiController {
    ...
    // ysh 관리자
    @PostMapping("/member-info/change-admin")
    public String changeAdminCode(@RequestParam("adminCode") String adminCode, RedirectAttributes redirectAttributes) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        Member currentMember = (Member) authentication.getPrincipal();

        try {
            memberService.changeAdminCode(currentMember.getId(), adminCode);
            redirectAttributes.addFlashAttribute("success", "관리자로 변경되었습니다..");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("error", "관리자 코드가 올바르지 않습니다.");  // 에러 메시지 변경
        }

        return "redirect:/member-info/" + currentMember.getId();  // Redirect back to the MemberInfo page
    }
